...
def main_func():
    pg.init()
    screen = pg.display.set_mode((int(window.winfo_screenwidth()), int(window.winfo_screenheight())))
    running = True
    clock = pg.time.Clock()
    ...
    while running:  # Handling events:
        ...
        surf = pg.transform.scale(surf, (int(window.winfo_screenwidth()), int(window.winfo_screenheight())))
        fps = int(clock.get_fps())
        pg.display.set_caption("FloorCasting - FPS: " + str(fps))
...
@njit()  # To increase FPS (Check it out)...
